...
import { ArrowLeft, Github, Check, ArrowUpRight, Eye, EyeOff, X } from "lucide-react";
...
export default function LoginPage() {
  ...
  return (
    // OUTER WRAPPER (Dikembalikan tanpa min-h-screen agar mengikuti parent layout)
    <div className="flex w-full px-4 sm:px-6 md:px-0 items-center justify-center">
      
      {/* CARD CONTAINER (Sama persis: max-w-3xl) */}
      <div className="flex w-full max-w-md md:max-w-3xl rounded-3xl border border-border/40 bg-background/60 backdrop-blur-2xl shadow-2xl overflow-hidden">
        ...
        {/* ================= LEFT SIDE (Sama persis spacingnya) ================= */}
          ...
          <div className="w-full max-w-md space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                <h1 className="text-2xl font-medium tracking-tight font-space">Secure Terminal Access</h1>
            ...
            <div className="flex items-center gap-4 my-7">
              ...
              <span className="text-[10px] tracking-[0.25em] uppercase text-muted-foreground/60 font-onest">or authenticate manually</span>
              <div className="flex-1 h-px bg-border" />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-0">
              ...
              <div className="relative">
                <label htmlFor="username" className={`absolute left-0 transition-all duration-300 ease-out pointer-events-none ${username ? "-top-3.5 text-[11px] text-muted-foreground/70" : "top-2 text-sm text-muted-foreground"}`}>Username or UserID</label>
                <input id="username" type="text" value={username} onChange={(e) => setUsername(e.target.value)} spellCheck={false} autoComplete="username" inputMode="text" className="w-full h-8 bg-transparent border-0 outline-none px-0 pr-8 text-sm leading-none focus:outline-none appearance-none" />
                ...
                <div className="absolute right-0 top-1/2 -translate-y-1/2">
                  {username.length > 3 ? <Check className="w-4 h-4 text-emerald-500 transition" /> : username.length > 0 ? <X className="w-4 h-4 text-red-500 transition" /> : null}
                </div>
              </div>

              {/* Password Field */}
              <div className="relative mt-7">
                <label htmlFor="password" className={`absolute left-0 transition-all duration-300 ease-out pointer-events-none ${password ? "-top-3.5 text-[11px] text-muted-foreground/70" : "top-2 text-sm text-muted-foreground"}`}>Secure Password</label>
                <input id="password" type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} spellCheck={false} autoComplete="current-password" className="w-full h-8 bg-transparent border-0 outline-none appearance-none px-0 pr-8 text-sm leading-none focus:outline-none" />
                <div className="absolute left-0 bottom-0 h-px w-full bg-muted-foreground/40" />
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-0 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-primary transition-transform duration-150 ease-out active:scale-95">
